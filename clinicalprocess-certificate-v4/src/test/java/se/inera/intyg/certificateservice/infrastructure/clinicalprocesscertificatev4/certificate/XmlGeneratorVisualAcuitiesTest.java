...
class XmlGeneratorVisualAcuitiesTest {
  ...
  void shallReturnEmptyListIfWrongElementValueType() {
    final var elementDataWithValueText =
        ElementData.builder().value(ElementValueText.builder().build()).build();

    final var actualResult =
        xmlGeneratorVisualAcuities.generate(elementDataWithValueText, ELEMENT_SPECIFICATION);
    assertEquals(Collections.emptyList(), actualResult);
  }

  @Test
  void shallReturnEmptyListIfVisualAcuitiesIsEmpty() {
    final var emptyVisualAcuities =
        ElementData.builder()
            .value(
    ...
    final var actualResult =
        xmlGeneratorVisualAcuities.generate(emptyVisualAcuities, ELEMENT_SPECIFICATION);
    assertEquals(Collections.emptyList(), actualResult);
  }
  ...
  class TestOneVisualAcuity {
    ...
    void shallReturnAnswerForRightEye() {
      ...
      answer.getDelsvar().addAll(List.of(buildDelsvar(ID_1, 2.0), buildDelsvar(ID_2, 1.0)));
                  ...
                  ElementValueVisualAcuities.builder()
                      .rightEye(
                          VisualAcuity.builder()
                              .withoutCorrection(
                                  Correction.builder().id(new FieldId(ID_1)).value(2.0).build())
                              .withCorrection(
                                  Correction.builder().id(new FieldId(ID_2)).value(1.0).build())
                              .build())
                      .leftEye(defaultEmptyVisualAcuity())
                      .binocular(defaultEmptyVisualAcuity())
                      .build())
      ...
      assertAll(
          ...
          () ->
              assertEquals(
                  "2,0", actualResult.getFirst().getDelsvar().getFirst().getContent().getFirst()),
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().getLast().getContent().getFirst()));
    }
    ...
    void shallReturnAnswerForBinocular() {
      ...
      final var actualResult =
          xmlGeneratorVisualAcuities.generate(emptyOcularAcuities, ELEMENT_SPECIFICATION);
  ...
  class TestMultipleOcularAcuities {
    ...
    void shallReturnAnswerWithAllValuesInCorrectOrder() {
              ...
              .value(
                  ElementValueVisualAcuities.builder()
                      .rightEye(
                          VisualAcuity.builder()
                              .withoutCorrection(
                                  Correction.builder().id(new FieldId(ID_1)).value(1.0).build())
                              .withCorrection(
                                  Correction.builder().id(new FieldId(ID_4)).value(1.0).build())
                              .build())
                      .leftEye(
                          VisualAcuity.builder()
                              .withoutCorrection(
                                  Correction.builder().id(new FieldId(ID_2)).value(1.0).build())
                              .withCorrection(
                                  Correction.builder().id(new FieldId(ID_5)).value(1.0).build())
                              .build())
                      .binocular(
                          VisualAcuity.builder()
                              .withoutCorrection(
                                  Correction.builder().id(new FieldId(ID_3)).value(1.0).build())
                              .withCorrection(
                                  Correction.builder().id(new FieldId(ID_6)).value(1.0).build())
                              .build())
                      .build())
              .build();
      ...
      assertAll(
          ...
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().get(0).getContent().getFirst()),
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().get(1).getContent().getFirst()),
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().get(2).getContent().getFirst()),
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().get(3).getContent().getFirst()),
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().get(4).getContent().getFirst()),
          () ->
              assertEquals(
                  "1,0", actualResult.getFirst().getDelsvar().get(5).getContent().getFirst()));
    }
  }
