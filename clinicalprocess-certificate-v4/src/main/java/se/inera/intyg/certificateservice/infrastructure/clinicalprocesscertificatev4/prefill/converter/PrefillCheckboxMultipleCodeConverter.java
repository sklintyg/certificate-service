...
public class PrefillCheckboxMultipleCodeConverter implements PrefillStandardConverter {
  ...
  public PrefillAnswer prefillAnswer(ElementSpecification specification, Forifyllnad prefill) {
    ...
    if (!(specification.configuration()
        instanceof ElementConfigurationCheckboxMultipleCode configurationCheckboxMultipleCode)) {
                            ...
                            .map(
                                answer -> {
                                  try {
                                    ...
                                    return getCodes(
                                        answer.getDelsvar(), configurationCheckboxMultipleCode);
                                  } catch (Exception e) {
                                    prefillErrors.add(
   ...
   *
   * <p>If {includeInXml} is {false}, we cannot prefill this value from the provided data.
   * Therefore, we prefill it with all values defined in the configuration.
   *
   ...
   */
  private static ElementData buildElementDataFromConfiguration(
  ...
  private static ElementValueCode getCodes(
      List<Delsvar> subAnswer, ElementConfigurationCheckboxMultipleCode configuration) {
    final var filteredSubAnswer =
        subAnswer.stream()
    ...
    final var listItem =
        configuration.list().stream()
            ...
            .findFirst()
            .orElseThrow(
                () -> new IllegalStateException("Could not find a matching code for " + code));

    return ElementValueCode.builder().codeId(listItem.id()).code(code.code()).build();
  }
}
