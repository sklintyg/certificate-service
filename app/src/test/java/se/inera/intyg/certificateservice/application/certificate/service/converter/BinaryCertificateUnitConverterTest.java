...
class BinaryCertificateUnitConverterTest {
  ...
  @BeforeEach
  void setUp() {
    binaryCertificateUnitConverter = new BinaryCertificateUnitConverter();
    careUnitBuilder =
        CareUnit.builder()
  ...
  @Test
  void shallConvertAddressFromElementData() {
    final var issuingUnit = careUnitBuilder.build();
    final var elementData = Optional.of(getElementData(EXPECTED_ADDRESS, null, null, null));
  ...
  void shallConvertCityFromElementData() {
    final var issuingUnit = careUnitBuilder.build();
    final var elementData = Optional.of(getElementData(null, EXPECTED_CITY, null, null));
    final var result = binaryCertificateUnitConverter.convert(issuingUnit, elementData);
    assertEquals(EXPECTED_CITY, result.getCity());
  }
  ...
  void shallConvertZipcodeFromElementData() {
    final var issuingUnit = careUnitBuilder.build();
    final var elementData = Optional.of(getElementData(null, null, EXPECTED_ZIP_CODE, null));
    final var result = binaryCertificateUnitConverter.convert(issuingUnit, elementData);
    assertEquals(EXPECTED_ZIP_CODE, result.getZipCode());
  }
  ...
  void shallConvertPhoneNumberFromElementData() {
    final var issuingUnit = careUnitBuilder.build();
    final var elementData = Optional.of(getElementData(null, null, null, EXPECTED_PHONE_NUMBER));
    final var result = binaryCertificateUnitConverter.convert(issuingUnit, elementData);
    assertEquals(EXPECTED_PHONE_NUMBER, result.getPhoneNumber());
  }
  ...
  private static ElementData getElementData(
    ...
    return ElementData.builder()
        .id(new ElementId(BinaryCertificateUnitConverterTest.ISSUING_UNIT))
        .value(
