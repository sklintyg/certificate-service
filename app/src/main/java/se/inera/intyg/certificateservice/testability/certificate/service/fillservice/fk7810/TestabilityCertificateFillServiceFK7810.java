...
@Component
public class TestabilityCertificateFillServiceFK7810 implements TestabilityCertificateFillService {
  ...
  private static List<ElementData> fillWithValues(
    ...
    final var specAktivitetsbegransning = spec(AKTIVITETSBAGRENSNINGAR_ID, certificateModel);
    ...
    sjukvardandeInsatsEgenvardInsatser(
        specSjukvardandeInsatserEgenvardInsatser, elementData, fillType);
    ovrigt(specOvrigt, elementData, fillType);

    return elementData;
  }

  private static void kannedomOmPatienten(
      ElementSpecification spec, ArrayList<ElementData> elementData) {
    if (emptyValue(spec) instanceof ElementValueCode) {
      elementData.add(
          elementData(
              spec.id(), valueCode(new FieldId(MINDRE_AN_ETT_AR.code()), MINDRE_AN_ETT_AR.code())));
    }
  }
    ...
    if (emptyValue(spec) instanceof ElementValueCodeList elementValueCodeList) {
      final var codes =
          List.of(
              ...
              valueCode(FUNKTIONSNEDSATTNING_ANDNINGS_ID, FUNKTIONSNEDSATTNING_ANDNINGS_ID.value()),
              valueCode(
  ...
  private static void aktivitetsbegransningar(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    if (emptyValue(spec) instanceof ElementValueCodeList elementValueCodeList) {
      final var codes =
          List.of(
              valueCode(
                  AKTIVITETSBAGRENSNINGAR_LARANDE_ID, AKTIVITETSBAGRENSNINGAR_LARANDE_ID.value()),
              valueCode(
                  AKTIVITETSBAGRENSNINGAR_KOMMUNIKATION_ID,
                  AKTIVITETSBAGRENSNINGAR_KOMMUNIKATION_ID.value()),
              valueCode(
                  AKTIVITETSBAGRENSNINGAR_MOVEMENT_ID, AKTIVITETSBAGRENSNINGAR_MOVEMENT_ID.value()),
              valueCode(
                  AKTIVITETSBAGRENSNINGAR_PERSONAL_CARE_ID,
                  AKTIVITETSBAGRENSNINGAR_PERSONAL_CARE_ID.value()),
              valueCode(
                  AKTIVITETSBAGRENSNINGAR_OVRIG_ID, AKTIVITETSBAGRENSNINGAR_OVRIG_ID.value()));
  ...
  private static void larande(ElementSpecification spec, List<ElementData> list) {
    if (emptyValue(spec) instanceof ElementValueText elementValueText) {
      final var text =
          elementValueText.withText("Lärande, tillämpa kunskap samt allmänna uppgifter och krav");
      list.add(elementData(spec.id(), text));
    }
  }

  private static void kommunikation(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    if (fillType == MAXIMAL && emptyValue(spec) instanceof ElementValueText elementValueText) {
  ...
  private static void forflyttning(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    if (fillType == MAXIMAL && emptyValue(spec) instanceof ElementValueText elementValueText) {
  ...
  private static void personligtVard(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    if (fillType == MAXIMAL && emptyValue(spec) instanceof ElementValueText elementValueText) {
  ...
  private static void sjukvardandeInsatsHSL(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    ...
    var value =
        fillType == MAXIMAL
            ...
            : ElementValueBoolean.builder()
                .booleanId(spec.configuration().id())
                .value(false)
                .build();
  ...
  private static void sjukvardandeInsatsHSLInsatser(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    if (fillType == MAXIMAL && emptyValue(spec) instanceof ElementValueText elementValueText) {
      final var text = elementValueText.withText("Ange vilka insatser och i vilken omfattning");
      list.add(elementData(spec.id(), text));
    }
  }

  private static void sjukvardandeInsatsEgenvard(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
  ...
  private static void sjukvardandeInsatsEgenvardInsatser(
      ElementSpecification spec, List<ElementData> list, TestabilityFillTypeDTO fillType) {
    if (fillType == MAXIMAL && emptyValue(spec) instanceof ElementValueText elementValueText) {
      final var text = elementValueText.withText("Ange vilka insatser och i vilken omfattning");
      list.add(elementData(spec.id(), text));
