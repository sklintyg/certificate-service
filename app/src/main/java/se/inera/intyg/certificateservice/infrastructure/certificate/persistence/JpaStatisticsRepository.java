...
public class JpaStatisticsRepository implements StatisticsRepository {
  ...
  @Override
  public Map<HsaId, UnitStatistics> getStatisticsForUnits(
      List<HsaId> issuedByUnitIds, List<CertificateModelId> allowedTypesToViewProtectedPerson) {

    final var certificateModelTypes =
        allowedTypesToViewProtectedPerson.stream()
    ...
    final var hsaIds = issuedByUnitIds.stream().map(HsaId::id).toList();

    final var draftCertificatesIssuedOnUnits =
        entityManager
            .createQuery(certificateJpql, Object[].class)
            .setParameter("hsaIds", hsaIds)
            .setParameter(
                "certificateStatusesForDrafts",
    ...
    final var messagesWithUnhandledQuestionsIssuedOnUnits =
        entityManager
            .createQuery(messageJpql, Object[].class)
            .setParameter("hsaIds", hsaIds)
            .setParameter(
                "messageStatusHandledOrDraft",
