...
class PdfConfigurationBooleanTest {
  ...
  private static final PdfFieldId CHECKBOX_TRUE = new PdfFieldId("form.true[0]");
  private static final PdfFieldId CHECKBOX_FALSE = new PdfFieldId("form.false[0]");

  private final Certificate certificate = mock(Certificate.class);

  @Test
  void shallReturnCheckedFieldWhenTrue() {
    ...
    final var config =
        PdfConfigurationBoolean.builder()
            .checkboxTrue(CHECKBOX_TRUE)
            .checkboxFalse(CHECKBOX_FALSE)
            .build();
  ...
  @Test
  void shallReturnFalseCheckboxWhenFalseAndConfigured() {
    final var elementValue =
  ...
  @Test
  void shallReturnEmptyWhenFalseAndNoCheckboxFalse() {
    final var elementValue =
        ElementValueBoolean.builder().booleanId(BOOLEAN_FIELD_ID).value(false).build();
    final var elementSpec = ElementSpecification.builder().id(ELEMENT_ID).build();
    final var config = PdfConfigurationBoolean.builder().checkboxTrue(CHECKBOX_TRUE).build();
  ...
  @Test
  void shallReturnEmptyWhenElementDataMissing() {
    final var elementSpec = ElementSpecification.builder().id(ELEMENT_ID).build();
    final var config =
        PdfConfigurationBoolean.builder()
            .checkboxTrue(CHECKBOX_TRUE)
