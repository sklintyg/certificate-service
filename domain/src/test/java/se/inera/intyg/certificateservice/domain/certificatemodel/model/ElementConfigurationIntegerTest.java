...
class ElementConfigurationIntegerTest {
  ...
  void shouldReturnSimplifiedValue() {
    final var value = ElementValueInteger.builder().value(42).build();

    final var config = ElementConfigurationInteger.builder().build();

    assertEquals("42", ((ElementSimplifiedValueText) config.simplified(value).get()).text());
  }
